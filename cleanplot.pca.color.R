'cleanplot.pca.color' <- 
  function(res.pca, grouping.factor = NULL, grouping.title = "Site", 
           ax1=1, ax2=2, scaling=2, plot.sites=TRUE, 
           plot.spe=TRUE, label.sites=TRUE, label.spe=TRUE, cex.char1=0.7,
           pos.sites=2, pos.spe=4, pos.legend = "topright", mult.spe=1, select.spe=NULL, 
    ...
    # grouping.factor  A factor use to color the sites points.
    # grouping.title   The name of the grouping variable.
...
# pos.legend.......Position of the legend box. One of "bottomright", "bottom", "bottomleft", "left", "topleft", "top", "topright", "right" and "center".
#
...
# Authors: Francois Gillet, Daniel Borcard & Pierre Legendre, 2016
{
  ...
  plot(mat[,c(ax1,ax2)], type="n", main=paste("PCA biplot - Scaling", scaling), xlim=c(lim[1,ax1], lim[2,ax1]), ylim=c(lim[1,ax2], lim[2,ax2]), 
       xlab=paste("PCA ",ax1, ", ", round(eig.val.rel[ax1], 3)*100,"%"), ylab=paste("PCA ",ax2, ", ", round(eig.val.rel[ax2], 3)*100,"%"), asp=1)
  ...
  if(plot.sites) {
    if(is.null(grouping.factor)){
      ...
      points(sit.sc[,ax1], sit.sc[,ax2], pch=20, col = as.numeric(grouping.factor))
      legend(x = pos.legend, pch=20, legend = levels(grouping.factor), col = unique(as.numeric(grouping.factor)), title = grouping.title)
    }
